...
class Constants(BaseConstants):
    ...
    instructions_template = 'discovery_info/Instructions_temp.html'


class Subsession(BaseSubsession):
    def creating_session(self):
        if self.session.config.get("cost_of_offer") == 5:
            with open('discovery_info/draws005_seed1.json', encoding='utf-8') as f:
                rows = json.load(f)  # 30*800
        else:
            with open('discovery_info/draws03_seed1.json', encoding='utf-8') as f:
                rows = json.load(f)  # 30*800

        with open('discovery_info/default_sequence_seed1.json', encoding='utf-8') as f:
